...
class GREPipeline(FilesPipeline):
    client = mangoDbUtil("GRE")
    ...
    def file_path(self, request, response=None, info=None):
        ...
        return "GRE\\"+join(basename(dirname(path)), basename(path))
                                ...
                                for i in range(len(article_html) - 1):
                                    if article_html[i:i + len("$img")] == "$img":
                                        if count == index:
                                            ...
                                            new_article_html += "<img src='upload/upload/img/gre/" + path_new + "'/>"
